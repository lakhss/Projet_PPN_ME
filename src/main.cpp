...
int main(int argc, char** argv) {
    ...
    if(choice == 8) {
        ...
        std::vector<int> n_trees_list = {10, 50, 100};
        int depth_bagging = 12;
        int depth_boosting = 4;
        ...
        for (int n : n_trees_list) {
            ...
            // Version HPC (S2)
            PerformanceEvaluator::evaluate_hpc(name, "Bagging HPC (N=" + std::to_string(n) + ")", [n, depth_bagging]() {
        ...
        // 2. BENCHMARK BOOSTING (Naïf vs HPC)
        for (int n : n_trees_list) {
            // Version Naïve (S1)
            PerformanceEvaluator::evaluate(name, "Boosting Naïf (N=" + std::to_string(n) + ")", [n, depth_boosting]() {
                BoostingRegressor b;
                b.n_estimators = n;
            ...
            // Version HPC (S2)
            PerformanceEvaluator::evaluate_hpc(name, "Boosting HPC (N=" + std::to_string(n) + ")", [n, depth_boosting]() {
